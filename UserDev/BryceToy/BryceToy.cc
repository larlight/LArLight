...
namespace larlight {
  ...
  bool BryceToy::initialize() {
    ...
    h = new TH2D("h1","Percent charge from dominant MCShower; IDE numElectrons; Fraction",100,0,4e7,102,-0.01,1.01);
  ...
  bool BryceToy::analyze(storage_manager* storage) {
    ...
    //////////////////
    std::map<unsigned int,larlight::simch> simch_map;    
    auto my_simch = (event_simch*)(storage->get_data(DATA::SimChannel));
    if(!my_simch){
      std::cerr<<"I found no simchannels!"<<std::endl;
    }
    //Add all simchannels to map along with channel number
    ...
    std::vector<float> hit_charge_breakdown;
    ...
    if(!my_clusters || !my_hits){
      std::cerr<<"I found an empty cluster!"<<std::endl;
    }
    ...
    for(auto this_cluster : *my_clusters) {      
      ...
      for(auto hit_index : hit_index_vector){
    	...
    	//Add total charge of hit to hit breakdown vector
    	hit_charge_breakdown.push_back(this_hit.Charge());
