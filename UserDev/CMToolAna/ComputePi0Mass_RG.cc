...
namespace larlight {
  ...
  bool ComputePi0Mass_RG::initialize() {
    ...
    hOpeningAngle = new TH2D("hPhotonOpening","PhotonAngle",60,0,3.14,60,0,3.14);
...
//	mc_shower->at(0).MotherAngle3D(phi,theta);
  ...
  //  hPi0MassPeak->Fill(_mass);
    ...
    hPi0MassPeakPull->Fill(_dorient_EE_calb*_mass -_Pi0mass);
    for(int i = 0; i<2; ++i){
 ...
 */   
	...
	if(_photon_dosReco>0.2 && _photon_dosReco<0.85) hPi0MassPeakdoscut->Fill(_dorient_EE_calb*_mass);
	if(recoangle>0.2 && recoangle<1.4) hPi0MassPeakanglecut->Fill(_dorient_EE_calb*_mass);
	if(recoangle>0.2 && recoangle<1.4&&_photon_dosReco>0.2 && _photon_dosReco<0.85) hPi0MassPeakdosanglecut->Fill(_dorient_EE_calb*_mass);
	}
