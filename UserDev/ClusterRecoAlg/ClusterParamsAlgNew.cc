...
namespace larreco {
  ...
  ClusterParamsAlg::ClusterParamsAlg(std::string mothermodule)
    : extname(mothermodule)//, fHBAlg()
  //    : extname(mothermodule),fHBAlg(pset.get< fhicl::ParameterSet >("HoughBaseAlg"))
  {
    linefittest_cm=nullptr;
    tgxtest=nullptr;
    hithistinv=nullptr;
    hitinv2=nullptr;
    hitreinv2=nullptr;
    ...
    tgxtest = new TH2F(Form("%s charge hi distrib",extname.c_str()),"charge hi distribution per wires",
		       10,0,10,10,0,10);
    funcname=Form("%s linefit",extname.c_str());   //new name for the function - module specific

    linefittest_cm = new TF1(funcname.c_str(),"pol1", 0,10); 
    
    std::string histname=Form("%s omega_single",extname.c_str()); 
    fh_omega_single = new TH1F(histname.c_str(),"Theta distribution Hit",720,-180., 180.) ;
  ...
  void ClusterParamsAlg::reconfigure() 
  {
    fChargeCutoffThreshold.push_back(500);
    fChargeCutoffThreshold.push_back(500);
    fChargeCutoffThreshold.push_back(1000);

    fSelectBoxSizePar  = 10;
    fSelectBoxSizePerp = 3;
    fMinHitListSize    = 40;
    fOutlierRadius     = 5;
    fForceRightGoing   = false;
    fHitDensityCutoff  = 1.9;
    fMultiHitWireCutoff = 2.1;
    fOffAxisHitsCutoff	= 0;
    fPrincipalComponentCutoff = 0;
    fShowerSelisORorAND = 0;
    
    //    fHBAlg.reconfigure();
    
    fWirePitch = larutil::Geometry::GetME()->WirePitch(0,1,0);    //wire pitch in cm
    fTimeTick  = larutil::DetectorProperties::GetME()->SamplingRate()/1000.; 
    
    //define conversion constants
    double fDriftVelocity=larutil::LArProperties::GetME()->DriftVelocity(larutil::LArProperties::GetME()->Efield(),
					...
					const std::vector<larlight::hit*> &hitlist) 
  {
    ...
    tgxtest->SetBins(nbinsx,((double)kMinWire-(double)wirepad)*fWiretoCm,
		     ((double)kMaxWire+(double)wirepad)*fWiretoCm,nbinsy,
		     (kMinTime-timepad)*fTimetoCm,(kMaxTime+timepad)*fTimetoCm);
    
    linefittest_cm->SetRange(((double)kMinWire-(double)wirepad)*fWiretoCm,
			     ((double)kMaxWire+(double)wirepad)*fWiretoCm); 
						 ...
						 double &wire_start,double &time_start,double &wire_end,double &time_end)
  {
    ...
    UInt_t startHit_index = gser->FindClosestHitIndex(hitlist, wire_online_begin,time_online_begin);
    UInt_t endHit_index = gser->FindClosestHitIndex(hitlist, wire_online_end,time_online_end);
				  ...
				  double lineintercept)
  {
    ...
    if(hithistinv) delete hithistinv;
					   ...
					   std::vector<larlight::hit*> &hitlist_high)
  {
	...
	break;
      ...
      this->SelectLocalHitlist(hitlist_high, hitlistlocal, (double)wire,time,fOutlierRadius);
					      ...
					      double  time_end, double lineslope)
  {
    ...
    int multihit_end=MultiHitWires(hitlistlocal_end);
					      ...
					      double  wire_end,double  time_end, double lineslope)
  {
    ...
    std::vector <UInt_t> hitlistlocal_start;
    std::vector <UInt_t> hitlistlocal_end;
						...
						double & wire_end,double & time_end, int &direction)
  {
    ...
    std::vector <std::vector<UShort_t> > houghlines; 
    std::vector <std::vector<UShort_t> > houghlinesend; 
    ...
    unsigned int iplane,wire;
    gser->GetPlaneAndTPC((*hitlist.begin()),iplane,wire);
    // 
    size_t numclus =0; 
	...
	for(unsigned int ix=0;ix<houghlinesend[endhoughclus].size();ix++) {
	  unsigned int w;
	  w = hitlistlocal_end.at(houghlinesend[endhoughclus][ix]).Wire(); 
	  
	  if(w*houghdirection>endwire*houghdirection) //multiply by original direction to find most extreme point 
	    {endwire=w;
	      endtime=hitlistlocal_end.at(houghlinesend[endhoughclus][ix]).PeakTime();
	    }
	}
	wire_end=endwire;
	time_end=endtime;
	...
	for(unsigned int ix=0;ix<houghlines[starthoughclus].size();ix++) {
	  ...
	  w = hitlistlocal_start.at(houghlines[starthoughclus][ix]).Wire(); 
	  
	  if(w*houghdirection<startwire*houghdirection) //multiply by original direction to find most extreme point 
	    {startwire=w;
	      starttime = hitlistlocal_start.at(houghlines[starthoughclus][ix]).PeakTime();
	    }
	}
