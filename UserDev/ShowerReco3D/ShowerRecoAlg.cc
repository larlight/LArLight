     ...
     if(wdist<fdEdxlength 
      ...
      || (direction==-1 && theHit.w<clustit.GetParams().start_point.w)  ) 
	     && ortdist<3.5 && linedist < fdEdxlength ){
	      ...
	      npoints_first++;
	      std::cout << " CALORIMETRY:" << " Pitch " <<newpitch << " dist: " << wdist <<  " dE/dx: " << dEdx_new << "MeV/cm "  << " average: " <<  dEdx_av/npoints_first  << "hit: wire, time " << theHit.w << " " << theHit.t << " line,ort " << linedist << " " << ortdist<< " direction " << direction << std::endl;
	     }
