...
namespace cluster {
  ...
  bool CMAlgoStartInPoly::Merge(const cluster_params &cluster1,
				     const cluster_params &cluster2)
  {
    ...
    if ( ( cluster2.PolyObject.PointInside( start1) ) and (cluster1.N_Hits > _MinHits) ){
      std::cout << "Start point of Cluster 1 in Polygon of Cluster 2!" << std::endl;
      return true;
    }
    //check if start point for cluster1 is in Polygon of cluster2
    if ( ( cluster1.PolyObject.PointInside( start2) ) and (cluster1.N_Hits > _MinHits) ){
