...
namespace cluster {
				...
				std::vector<std::vector<larutil::PxHit> > &pxhits_v) const
  {
    ...
    case ::larlight::DATA::RyanCluster:
      break;
    default:
      throw CRUException(Form("Not cluster data type: %s",
    ...
    if(!ev_cluster)
      ...
      throw CRUException(Form("Cluster type not in storage: %s",
			      ::larlight::DATA::DATA_TREE_NAME[cluster_type].c_str()
			      )
			 );
    ...
    if(!ev_hits) 
      ...
      throw CRUException(Form("Associated hit type not in storage: %s",
