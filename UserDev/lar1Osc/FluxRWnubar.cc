...
namespace lar1{
  ...
  {
    std::cout <<"Initializing FluxRWnubar"<<std::endl;
    ...
    mbhistfile.append("FluxRWnubar/hist/december2007_horn-174ka_rgen610.6_flux.root");
    ...
    for (Int_t iptype=1;iptype<=4;iptype++) {
      ...
      //Need to scale to get to nu/m^2/1e7POT/50MeV (gshist is normalized 1e6POT*30 (factor of 30 because using 30 redecays))                           
      Double_t scale = 
      (1./(std::pow(610.6,2)*3.14159)) //events spread over A=610.6^2 cm2
                            * (1.0e4)                  //cm2->m2
                            * (1.0e7)*30.;                   //scale to 1e7 POT (GShist 1e6POT*30redecay)
    ...
    gshistfile.append(Form("/FluxRWnubar/hist/%s_nubarntp.root",nu[intype-1].c_str()));
  ...
  Double_t FluxRWnubar::GetWeight(Double_t energy, Int_t ntype, Int_t ptype)
  {
    Double_t wgh=1;
  ...
  TH1D* FluxRWnubar::GetWeightHist(Int_t ntype, Int_t ptype) 
  {
    if (ntype>4 || ntype<1) {
    std::cout <<"Invalid neutrino type ntype="<<ntype<<" ! Expected 1,2,3 or 4 (nue, nuebar, numu or numubar)"<<std::endl;
    } else if (ptype>4 || ptype<1) {
  ...
  std::string FluxRWnubar::GetEnv( const std::string & var ){
