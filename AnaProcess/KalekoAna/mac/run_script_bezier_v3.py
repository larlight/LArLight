...
in_fname = [ "v3_ang0_dsout_reduced_bezier.root",
             "v3_ang30_dsout_reduced_bezier.root",
             "v3_ang45_dsout_reduced_bezier.root",
             "v3_ang60_dsout_reduced_bezier.root",
             "v3_ang90_dsout_reduced_bezier.root" ]
...
def plot_shit():
    ...
    for x in xrange(len(in_angles)):
        ...
        recohist_array[x].SetTitle("Bezier Muon Reco [Initial ThetaYZ - MCTheta]: MCTheta = %d, nMuons = %d;Initial Track ThetaYZ - MCTheta (deg);Counts" % (in_angles[x],mchist_array[x].GetEntries()))
        ...
        legend.Clear()
        legend.AddEntry(recohist_array[x],"Bezier Reco")
        ...
        canvas.SaveAs(out_dir+"/angle_%d_recotheta_effgraph_bezier.png" % in_angles[x])
    ...
    for x in xrange(len(in_angles)):
        nreco_array[x].GetXaxis().SetTitle("Number Bezier Reconstructed Tracks (MCtruth: 1)")
        nreco_array[x].Draw()
        canvas2.Modified(); canvas2.Update()
    
        canvas2.SaveAs(out_dir+"/angle_%d_multiplicity_bezier.png" % in_angles[x])
